...
export const isAuthor: RequestHandler = (req: any, res, next) => {
    if(req.user) {     //mobile app doesn't seem to be sending a req.user, so for now given that the info here is not sensitive, the mobile requests will be sent through the 2nd part of this if/else
        ...
        } else if(req.user.role === 'author' && req.params.id && req.method !== 'DELETE') {  //verifies that 'author' roles can only edit their own posts, but 'admin' and 'webmaster' can do so with any post
            if(req.user.userid != req.body.authorid) {                                       //excluding delete is for convenience in getting mobile app functioning, obviously security problem in a real world thing
                return res.sendStatus(401);
            } else {
                return next();
            }
        ...
        if(!req.body.role || req.body.role === 'guest') {
            return res.sendStatus(401);
        } else if(req.body.role === 'author' && req.params.id && req.method !== 'DELETE') { 
